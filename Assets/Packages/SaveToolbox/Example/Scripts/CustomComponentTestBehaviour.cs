...
namespace SaveToolbox.Example.Scripts
{
	...
	public class CustomComponentTestBehaviour : MonoBehaviour
	{
		...
		[ContextMenu("Log collection values")]
		private void LogCollections()
		{
			...
			foreach (var i in intHashSet)
			{
				Debug.Log($"int hash set: {i}");
			}

			foreach (var i in stringHashSet)
			{
				Debug.Log($"string hash set: {i}");
			}

			foreach (var kvp in intDictionary)
			{
				Debug.Log($"int dictionary: {kvp.Key}, {kvp.Value}");
			}

			foreach (var kvp in stringDictionary)
			{
				Debug.Log($"string dictionary: {kvp.Key}, {kvp.Value}");
			}
		}

		[ContextMenu("Clear collections")]
