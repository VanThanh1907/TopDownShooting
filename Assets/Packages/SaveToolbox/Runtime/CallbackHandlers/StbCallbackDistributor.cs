...
namespace SaveToolbox.Runtime.CallbackHandlers
{
	...
	public class StbCallbackDistributor
	{
		...
		public async Task CacheCallbackTypesAsync()
		{
			var allAssemblies = AppDomain.CurrentDomain.GetAssemblies();
			var allAssemblyTypes = new List<Type>();
			foreach (var assembly in allAssemblies)
			{
				allAssemblyTypes.AddRange(assembly.GetTypes());
				await AsynchronousUtilities.CheckFrameTime();
			}

			loadedCallbackTypes = await ReturnTypesFromList<IStbLoadedCallbackHandler>();
			beforeLoadedCallbackTypes = await ReturnTypesFromList<IStbBeforeLoadedCallbackHandler>();
			savedCallbackTypes = await ReturnTypesFromList<IStbSavedCallbackHandler>();
			beforeSavedCallbackTypes = await ReturnTypesFromList<IStbBeforeSavedCallbackHandler>();

			async Task<IEnumerable<Type>> ReturnTypesFromList<T>() where T : class
			{
				var returnList = new List<Type>();
				foreach (var assemblyType in allAssemblyTypes)
				{
					if (typeof(MonoBehaviour).IsAssignableFrom(assemblyType) && assemblyType.GetInterfaces().Contains(typeof(T)) && !assemblyType.IsInterface)
					{
						returnList.Add(assemblyType);
					}
					await AsynchronousUtilities.CheckFrameTime();
				}
		...
		public void HandleSaved(SaveData slotSaveData, bool allowInactiveMonoBehaviours = true)
		{
			foreach (var savedCallbackType in savedCallbackTypes)
			{
				...
				var sortMode = allowInactiveMonoBehaviours ? FindObjectsInactive.Include : FindObjectsInactive.Exclude;
				var savedCallbackObjects = Object.FindObjectsByType(savedCallbackType, sortMode, FindObjectsSortMode.None);
...
#if UNITY_6000_0_OR_NEWER
				var sortMode = allowInactiveMonoBehaviours ? FindObjectsInactive.Include : FindObjectsInactive.Exclude;
				var loadedCallbackObjects = Object.FindObjectsByType(loadedCallbackType, sortMode, FindObjectsSortMode.None);
#elif UNITY_2021_3_OR_NEWER
