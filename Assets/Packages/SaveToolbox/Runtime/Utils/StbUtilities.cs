	...
	public static class StbUtilities
	{
		...
		public static object GetMemberInfoValue(this MemberInfo memberInfo, object targetObject)
		{
			switch (memberInfo)
			{
				...
				default:
					Debug.LogWarning("[SaveToolbox] Tried to get a value from a type that wasn't a member info.");
					return null;
			}
		}
		...
		public static void SetMemberInfoValue(this MemberInfo memberInfo, object targetObject, object value)
		{
			switch (memberInfo)
			{
				...
				case PropertyInfo propertyInfo:
					...
					break;
				default:
					Debug.LogWarning("[SaveToolbox] Tried to set a value from a type that wasn't a member info.");
					break;
			}
		}
		...
		public static Type GetMemberInfoType(this MemberInfo memberInfo)
		{
			switch (memberInfo)
			{
				...
				default:
					Debug.LogWarning("[SaveToolbox] Tried to get a type from a type that wasn't a member info.");
					return null;
			}
		}
...
#if STB_USE_ADDRESSABLES
		public static async Task<T> LoadAssetAsync<T>(string label)
		{
			var asyncOperation = Addressables.LoadAssetAsync<T>(label);
			...
			Debug.LogError($"[SaveToolbox] Failed to load label: {label}");
			return default;
		}
#endif
	}
