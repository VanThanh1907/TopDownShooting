	...
	public class StbDataContainer
	{
		...
		public void AddKeyValueEntry(string key, object value)
		{
			foreach (var stbDataElement in serializableObjectsList)
			{
				...
				{
					Debug.LogError("[SaveToolbox] Could not add key value pair to StbDataContainer. Ensure a object of the same key does not already exist.");
					return;
				}
			}
		...
		public T GetValue<T>(string key)
		{
			foreach (var stbDataElement in serializableObjectsList)
			{
				if (stbDataElement.Key == key)
				{
					...
					Debug.LogError($"[SaveToolbox] Tried to get a value that was not of the correct type of: {typeof(T)}");
				}
			}
