	...
	public class StbJsonSerializer
	{
		...
		private async Task<JsonBaseNode> SerializeObjectThroughReflectionAsync(Type objectType, object objectValue)
		{
			...
			try
			{
				...
				foreach (var memberInfo in membersInfo)
				{
					...
					switch (memberInfo.MemberType)
					{
						...
						case MemberTypes.Property:
							...
							break;
						default:
							if (SaveToolboxPreferences.Instance.LoggingEnabled) Debug.LogWarning($"[SaveToolbox] Cannot serialize member of type {memberInfo.MemberType}.");
							break;
					}
				...
				if (SaveToolboxPreferences.Instance.LoggingEnabled && jsonObjectNode.Children.Count == 0) Debug.LogWarning($"[SaveToolbox] Tried to serialize object but it was empty. Type was :{objectType}");
				return jsonObjectNode;
			}
			catch (Exception e)
			{
				Debug.LogError($"[SaveToolbox] Failed Serialization with error: {e}");
			}
		...
		private async Task<JsonCompoundNode> SerializeCollectionAsync(Type objectType, object objectValue)
		{
			...
			if (isCollection)
			{
				var collection = (IEnumerable)objectValue;
				var isStack = objectType.IsStack();
				var isDictionary = objectType.IsDictionary();

				if (isDictionary)
				{
					foreach (var listObject in collection)
					{
						var keyValueType = listObject.GetType();
						var keyType = listObject.GetType().GetGenericArguments()[0];
						var valueType = listObject.GetType().GetGenericArguments()[1];

						var key = keyValueType.GetProperty("Key");
						var value = keyValueType.GetProperty("Value");

						var currentKey = key.GetValue(listObject);
						var currentValue = value.GetValue(listObject);

						var newKeyChild = await SerializeObjectAsync(keyType, currentKey);
						var newValueChild = await SerializeObjectAsync(valueType, currentValue);
						arrayNode.AddChild(newKeyChild);
						arrayNode.AddChild(newValueChild);
					}
				}
				else if (isStack)
				{
			...
			if (isCollection)
			{
				...
				if (isDictionary)
				{
					foreach (var listObject in collection)
					{
						...
						var newKeyChild = SerializeObject(keyType, currentKey);
						var newValueChild = SerializeObject(valueType, currentValue);
						arrayNode.AddChild(newKeyChild);
						arrayNode.AddChild(newValueChild);
					}
				}
		...
		private object DeserializeCollection(Type collectionType, JsonBaseNode jsonBaseNode)
		{
			...
			var isHashSet = collectionType.IsHashSet();
			var isDictionary = collectionType.IsDictionary();
			...
			if (isHashSet)
			{
				...
				var queueElementType = collectionType.GetGenericArguments()[0];
				var hashSetInstance = (IEnumerable)Activator.CreateInstance(typeof(HashSet<>).MakeGenericType(queueElementType));
				var enqueueMethod = hashSetInstance.GetType().GetMethod("Add");
				...
				for (var index = 0; index < listElements.Count; index++)
				{
					enqueueMethod?.Invoke(hashSetInstance, new object[] { DeserializeObject(queueElementType, listElements[index]) });
				}
			...
			if (isDictionary)
			{
				...
				var dictionaryInstance = (IEnumerable)Activator.CreateInstance(typeof(Dictionary<,>).MakeGenericType(keyType, valueType));
				var addMethod = dictionaryInstance.GetType().GetMethod("Add");
				...
				for (var index = 0; index < listElements.Count; index++)
				{
					var keyObject = DeserializeObject(keyType, listElements[index]);
					var valueObject = DeserializeObject(valueType, listElements[++index]);
					addMethod?.Invoke(dictionaryInstance, new object[] { keyObject, valueObject });
				}
		...
		private async Task<object> DeserializeCollectionAsync(Type collectionType, JsonBaseNode jsonBaseNode)
		{
			...
			if (isHashSet)
			{
				...
				var queueElementType = collectionType.GetGenericArguments()[0];
				var queueInstance = (IEnumerable)Activator.CreateInstance(typeof(HashSet<>).MakeGenericType(queueElementType));
				var addMethod = queueInstance.GetType().GetMethod("Add");
				...
				for (var index = 0; index < listElements.Count; index++)
				{
					addMethod?.Invoke(queueInstance, new object[] { await DeserializeObjectAsync(queueElementType, listElements[index]) });
				}
			...
			if (isDictionary)
			{
				...
				for (var index = 0; index < listElements.Count; index++)
				{
					var keyObject = await DeserializeObjectAsync(keyType, listElements[index]);
					var valueObject = await DeserializeObjectAsync(valueType, listElements[++index]);
					addMethod?.Invoke(dictionaryInstance, new object[] { keyObject, valueObject });
				}
		...
		protected virtual object Deserialize(Type type, JsonCompoundNode jsonCompoundNode)
		{
			...
			if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(SaveEntityReference<>))
			{
				...
				Debug.LogWarning($"[SaveToolbox] Could not find a instance of an ISaveDataEntity with the identifier: {identifierString}");
				return default;
			}
