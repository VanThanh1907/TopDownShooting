				...
				foreach (var memberInfo in membersInfo)
				{
					...
					switch (memberInfo.MemberType)
					{
						...
						case MemberTypes.Property:
							...
							break;
						default:
							if (SaveToolboxPreferences.Instance.LoggingEnabled) Debug.LogWarning($"[SaveToolbox] Cannot serialize member of expectedType {memberInfo.MemberType}.");
							break;
					}
			...
			if (isCollection)
			{
				...
				if (isDictionary)
				{
					foreach (var listObject in collection)
					{
						...
						SerializeObject(keyType, currentKey, byteWriter);
						SerializeObject(valueType, currentValue, byteWriter);
					}
				}
				else if (isStack)
				{
			...
			if (isCollection)
			{
				...
				if (isDictionary)
				{
					foreach (var listObject in collection)
					{
						...
						await SerializeObjectAsync(keyType, currentKey, byteWriter);
						await SerializeObjectAsync(valueType, currentValue, byteWriter);
					}
				}
				else if (isStack)
				{
		...
		private async Task<object> DeserializeCollectionAsync(Type collectionType, StbByteReader byteReader)
		{
			...
			if (isSerializableCollection)
			{
				...
				if (isQueue)
				{
					...
					var hashSetType = collectionType.GetGenericArguments()[0];
					var hashSetInstance = (ICollection)Activator.CreateInstance(typeof(Queue<>).MakeGenericType(hashSetType));
					var enqueueMethod = hashSetInstance.GetType().GetMethod("Enqueue");

					// Populate list.
					for (var i = 0; i < length; i++)
					{
						enqueueMethod?.Invoke(hashSetInstance, new object[] { await DeserializeObjectAsync(hashSetType, byteReader) });
					}
				...
				if (isHashSet)
				{
					...
					for (var i = 0; i < length; i++)
					{
						addMethod?.Invoke(queueInstance, new object[] { await DeserializeObjectAsync(queueElementType, byteReader) });
					}
				...
				if (isDictionary)
				{
					...
					for (var i = 0; i < length; i++)
					{
						var key = await DeserializeObjectAsync(keyType, byteReader);
						var value = await DeserializeObjectAsync(valueType, byteReader);
		...
		private object DeserializeCollection(Type collectionType, StbByteReader byteReader)
		{
			...
			if (isSerializableCollection)
			{
				...
				if (isHashSet)
				{
					...
					var hashSetElementType = collectionType.GetGenericArguments()[0];
					var hashSetInstance = (IEnumerable)Activator.CreateInstance(typeof(HashSet<>).MakeGenericType(hashSetElementType));
					var addMethod = hashSetInstance.GetType().GetMethod("Add");

					// Populate list.
					for (var i = 0; i < length; i++)
					{
						addMethod?.Invoke(hashSetInstance, new object[] { DeserializeObject(hashSetElementType, byteReader) });
					}
				...
				if (isDictionary)
				{
					...
					for (var i = 0; i < length; i++)
					{
						var key = DeserializeObject(keyType, byteReader);
						var value = DeserializeObject(valueType, byteReader);
						addMethod?.Invoke(dictionaryInstance, new object[] { key, value});
					}
