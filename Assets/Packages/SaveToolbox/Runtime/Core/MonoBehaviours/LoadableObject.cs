	...
	public class LoadableObject : MonoBehaviour
	{
...
#if UNITY_2021_3_OR_NEWER // Non reorderable not available on older editors.
		...
		public LoadableObjectSaveData Serialize(SaveSettings saveSettings)
		{
			...
			for (var i = 0; i < saveDataEntityBehaviours.Count; i++)
			{
				...
				{
					if (SaveToolboxPreferences.Instance.LoggingEnabled) Debug.LogWarning("[SaveToolbox] Tried to serialize a non-ISaveDataEntity. This should not be in the loadable objects saveDataEntityBehaviours.");
					continue;
				}
		...
		public void Deserialize(LoadableObjectSaveData loadableObjectSaveData)
		{
			...
			foreach (var saveEntityObjectData in loadableObjectSaveData.SaveDataObjects)
			{
				...
				{
					if (SaveToolboxPreferences.Instance.LoggingEnabled) Debug.LogWarning("[SaveToolbox] Tried to deserialize a non-ISaveDataEntity. This should not be in the loadable objects saveDataEntityBehaviours.");
					continue;
				}
