...
public class ProjectileMoveScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Sử dụng pool cho muzzlePrefab
        if (muzzlePrefab != null)
        {
            var muzzleVFX = MyPoolManager.Instance.Get(muzzlePrefab, transform.position);
    ...
    void OnCollisionEnter(Collision co)
    {
        if (!bounce)
        {
            if (co.gameObject.tag != "Bullet" && !collided)
            {
                ...
                // Sử dụng pool cho hitPrefab
                if (hitPrefab != null)
                {
                    var hitVFX = MyPoolManager.Instance.Get(hitPrefab, pos);
                ...
                // Thay Destroy bằng SetActive(false) để trả về pool
        ...
        else
        {
            ...
            Destroy(this); // Lưu ý: Với bounce, cần xử lý pool riêng nếu muốn tái sử dụng
        }
    }
